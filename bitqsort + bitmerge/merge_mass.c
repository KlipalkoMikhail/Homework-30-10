...
void merge_mass(int C[], int l, int m, int r, int k)             // функция слияния двух массивов
{
    ...
    for(int s = l; s <= r; s++)                                     // цикл слияния двух массивов начинаем с левого указателя
    {
        ...
        else if (to_binary(A[i], &u1) <= to_binary(B[j], &u2))                                      // берем число из первого массива и сравниваем со вторым
        {
            C[s] = A[i];
            i++;
        }
